...
def dashboard(request):
    ...
    active_tickets = Ticket.objects.select_related('queue').exclude(
        status__in=[Ticket.COMPLETED_STATUS, Ticket.COMPLETED_STATUS],
    )
...
def report_index(request):
    ...
    for queue in Queues:
        dash_ticket = {
            ...
            'completed': queue.ticket_set.filter(status=3).count(),
            'reopened': queue.ticket_set.filter(status=2).count(),
            'pending': queue.ticket_set.filter(status=4).count(),
